...
public class ActivityDaoImpl implements ActivityDao {
	...
	public int insert(ActivityComment item) throws SQLException {
		String sql = "insert into " + ActivityComment.TABLE_NAME + " (" +
				...
				ActivityComment.COM_DATE + ", " + ActivityComment.COM_CON + ", " +
	...
	public List<ActivityComment> findAllComment(Integer actId) throws Exception {
		...
		try (
			...
			while (rs.next()) {
				...
				message.setMessage(rs.getString(ActivityComment.COM_CON));
				message.setUpdateDate(rs.getTimestamp(ActivityComment.COM_DATE));
				comment.setComment(message);
				Message reply = new Message();
				reply.setMessage(rs.getString(ActivityComment.REP_CON));
				reply.setUpdateDate(rs.getTimestamp(ActivityComment.REP_DATE));
	...
	public List<Activity> findAll(String userId) throws SQLException {
		String sql = "select A.*, (CASE WHEN M.USER_NO IS NOT NULL THEN true ELSE false END) as isCollection ," + 
				" (select COUNT(*) from ACTI_COMM AS AC where AC.ACTI_NO = A.ACTI_NO) as commcount" + 
